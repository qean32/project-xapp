...
import { swapAddedNotification, swapLargeFileNotification, swapDownloadFileNotification } from '../../store/notification'
...
export const Notification: React.FC<Props> = ({ }: Props) => {
    const { added, download, largeFile } = useAppSelector(state => state.notification)
    ...
    React.useEffect(() => {
        if (largeFile)
            setTimeout(() => dispatch(swapLargeFileNotification()), 2100)
    }, [largeFile])
    React.useEffect(() => {
        if (added)
    ...
    return (
        <>
            <NotificationHoc view={(!!(current.link == largeFileMessage) || largeFile)} >
